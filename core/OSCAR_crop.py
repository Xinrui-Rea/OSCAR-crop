...
def CROP(option='offline', use_MIT=True, use_Geo=False, use_2nd=False, use_MM=False, **useless):
    ...
    '''
    assert sum([use_MIT, use_Geo, use_2nd, use_MM]) == 1, 'Please choose one function for N-yield response!'
    ...
    model.process(
        Out = 'RC', 
        In = ('D_CO2', ),
        Eq = lambda Var, Par: Eq__RC(Var, Par),
        units='1')
    ...
    model.process(
        Out = 'D_Tgs', 
        In = ('D_Tl', ),
        Eq = lambda Var, Par: Eq__D_Tgs(Var, Par),
        units='K')
    ...
    model.process(
        Out = 'RT', 
        In = ('D_Tgs', ),
        Eq = lambda Var, Par: Eq__RT(Var, Par),
        units='1')

    def Eq__RT(Var, Par):
        RT = xr.full_like(Var.D_Tgs, np.nan)
    ...
    model.process(
        Out = 'D_Pgs', 
        In = ('D_Pl', ),
        Eq = lambda Var, Par: Eq__D_Pgs(Var, Par),
        units='mm yr-1')
    ...
    model.process(
        Out = 'RP', 
        In = ('D_Pgs', ),
        Eq = lambda Var, Par: Eq__RP(Var, Par),
        units='1')
    ...
    model.process(
        Out = 'NI', 
        In = ('N_fertl', 'N_dep'),
        Eq = lambda Var, Par: Eq__NI(Var, Par),
        units='kgN ha-1 yr-1')
    ...
    def Eq__RN(Var, Par):
        ...
        RN = RN.where(RN > 0)
        RN = xr.where(RN.spc_crop == 'soy', 1.0, RN)
        return RN
    ...
    def Eq__CY(Var, Par):
        return Par.CY_0 * Var.RC * Var.RT * Var.RP * Var.RN
    ...
    model.process(
        Out = 'BY', 
        In = ('CY', ),
        Eq = lambda Var, Par: Eq__BY(Var, Par),
        units='tDM ha-1')
    
    def Eq__BY(Var, Par):
        return Var.CY / Par.HI

    ## crop residue
    model.process(
        Out = 'RY', 
        In = ('BY', ),
        Eq = lambda Var, Par: Eq__RY(Var, Par),
        units='tDM ha-1')

    def Eq__RY(Var, Par):
        return Var.BY * (1 - Par.HI)

    ...
    **useless):
    ...
    if mod_region == 'sub-national':
        for data in datasets:
            if os.path.isfile(f'{path_data}drivers/nitrogen_fertl__{data}_{mod_region}.nc'):
                data_dict[data] = f'{data}_{mod_region}' 
    if mod_region == 'national': 
        for data in datasets:
            for region in ['national', 'sub-national']:
                if os.path.isfile(f'{path_data}drivers/nitrogen_fertl__{data}_{region}.nc'):
    ...
    for data, name in data_dict.items():
        ...
        For1 = xr.load_dataset(f'{path_data}drivers/nitrogen_fertl__{name}.nc')
        ...
        if data in ['Adalibieke_2023']:
            For1['N_fertl'] = xr.DataArray(np.nansum([For1[var] for var in For1.data_vars], axis=0), dims=('year', 'spc_crop', 'reg_code'))
            for spc in crop_species:
                spc_old = spc
                if spc in ['ri1', 'ri2']: spc_old = 'ric'
                if spc in ['swh', 'wwh']: spc_old = 'whe'
...
def load_Ndep_hist(datasets=['ISIMIP3b'],
    ...
    **useless):
    ...
    if mod_region == 'sub-national':
        for data in datasets:
            if os.path.isfile(f'{path_data}drivers/nitrogen_dep__{data}_{mod_region}.nc'):
                data_dict[data] = f'{data}_{mod_region}' 
    if mod_region == 'national': 
        for data in datasets:
            for region in ['national', 'sub-national']:
                if os.path.isfile(f'{path_data}drivers/nitrogen_dep__{data}_{region}.nc'):
                    data_dict[data] = f'{data}_{region}'
    datasets_missing = [data for data in datasets if data not in data_dict.keys()]
    if len(datasets_missing) > 0:
        print(f'The following datasets are not compatible: {datasets_missing}')
    ...
    for data, name in data_dict.items():
        print(f'Loading nitrogen_fertl__{name} ...')
        For1 = xr.load_dataset(f'{path_data}drivers/nitrogen_dep__{name}.nc')
    ...
    ## order dimensions
    For = For.transpose(*(['year', 'reg_code'] + [var for var in For.coords if 'data' in var]))
...
def load_Nfertl_scen(datasets=['Jagermeyr_2021', 'ISIMIP3b-5crops'],
    ...
    **useless):
    ...
    ## order dimensions
    For = For.transpose(*(['year', 'spc_crop', 'reg_code'] + [var for var in For.coords if 'scen' in var]))
...
def load_Ndep_scen(datasets=['ISIMIP3b'],
    ...
    **useless):
    ...
    ## order dimensions
    For = For.transpose(*(['year', 'reg_code'] + [var for var in For.coords if 'scen' in var]))
...
def load_harvested_hist(datasets=['Adalibieke_2023', 'Waha_2020', 'Becker-Reshef_2023', 'CROPGRIDSv1.07', 'RiceAtlas', 'MIRCA2000', 'Monfreda_2008', 'ric-ISIMIP3b', 'FAO'],
    ...
    **useless):
    ...
    data_dict = {}
    ## check regional level
    if mod_region.lower() == 'sub-national':
        for data in datasets:
            if os.path.isfile(f'{path_data}drivers/harvested-area__{data}_{mod_region}.nc'):
                data_dict[data] = f'{data}_{mod_region}' 
    if mod_region.lower() == 'national': 
        for data in datasets:
            for region in ['national', 'sub-national']:
                if os.path.isfile(f'{path_data}drivers/harvested-area__{data}_{region}.nc'):
                    data_dict[data] = f'{data}_{region}'
    datasets_missing = [data for data in datasets if data not in data_dict.keys()]
    if len(datasets_missing) > 0:
        print(f'The following datasets are not compatible: {datasets_missing}')
    ...
    for data, name in data_dict.items():
        ...
        For1 = xr.load_dataset(f'{path_data}drivers/harvested-area__{name}.nc')
        ...
        if data in ['Adalibieke_2023', 'Waha_2020', 'Becker-Reshef_2023', 'MIRCA2000', 'Monfreda_2008', 'ric-ISIMIP3b']:
            ...
            For1 = For1['Ah']
            if mod_region.lower() == 'national':
                For1_na = aggreg_region(For1.to_dataset(), new_region=mod_region.capitalize(), weight_dict={'Ah': 1})['Ah']
                For1 = For1_na.rename({'reg_code_new': 'reg_code'})
    ...
    **useless):
    ...
    if mod_region.lower() == 'sub-national':
        if irr_flag:
            ...
            for data in datasets:
                if os.path.isfile(f'{path_data}drivers/cropland__{data}_{mod_region}.nc'):
                    data_dict[data] = f'{data}_{mod_region}' 
    elif mod_region.lower() == 'national': 
        if irr_flag:
            data_dict['ISIMIP3b-15crops'] = 'ISIMIP3b-15crops_sub-national'
        else:
            for data in datasets:
                for region in ['national', 'sub-national']:
                    if os.path.isfile(f'{path_data}drivers/cropland__{data}_{region}.nc'):
                        data_dict[data] = f'{data}_{region}'
    ...
    for data, name in data_dict.items():
        ...
        For1 = xr.load_dataset(f'{path_data}drivers/cropland__{name}.nc')
        ...
        if data == 'ISIMIP3b-15crops':
            ...
            if mod_region.lower() == 'national': 
                For1_na = aggreg_region(For1.to_dataset(), new_region=mod_region.capitalize(), weight_dict={'Ac': 1})['Ac']
                For1 = For1_na.rename({'reg_code_new': 'reg_code'})
        ...
        if data in ['Jackson_2019', 'CROPGRIDSv1.07']:
            ...
            if mod_region.lower() == 'national' and data in ['Jackson_2019']: 
                For1_na = aggreg_region(For1.to_dataset(), new_region=mod_region.capitalize())['Ac']
                For1 = For1_na
    ...
    ## order dimensions
    if irr_flag:
        For = For.transpose(*(['year', 'spc_crop', 'irr', 'reg_code'] + [var for var in For.coords if 'data' in var]))
    else:
        For = For.transpose(*(['year', 'spc_crop', 'reg_code'] + [var for var in For.coords if 'data' in var]))
...
def load_cropland_scen(datasets=['ISIMIP3b-15crops'],
    ...
    **useless):
    ...
    for data, name in data_dict.items():
        ...
        For1 = xr.load_dataset(f'{path_data}drivers/cropland__{data}.nc')
    ...
    ## order dimensions
    if irr_flag:
        For = For.transpose(*([var for var in For.coords if var == 'year'] + ['reg_code','spc_crop', 'irr'] + [var for var in For.coords if 'scen' in var]))
    else:
        For = For.transpose(*([var for var in For.coords if var == 'year'] + ['reg_code', 'spc_crop'] + [var for var in For.coords if 'scen' in var]))
